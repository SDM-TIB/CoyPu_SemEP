...
query_disaster = prefixes + """
SELECT DISTINCT ?countryCode ?startDate ?endDate ?year ?countryLocation ?location ?eventType ?alertScore ?affectedPopulation ?geometry ?alertLevel ?eventStatus ?intensity ?confidence ?resolution ?label ?comment WHERE {
  ?s a coy:Disaster .
  ?s <https://www.geonames.org/ontology#countryCode> ?countryCode .
  ?s coy:hasStartDate ?startDate .
  ?s coy:hasEndDate ?endDate .
  ?s <http://www.w3.org/2006/time#year> ?year .
  ?s coy:hasCountryLocation ?countryLocation .
  ?s coy:hasLocation ?location .

  optional{?s coy:hasEventType ?eventType .}
  optional{?s coy:hasAlertScore ?alertScore .}
  optional{?s coy:hasAffectedPopulation ?affectedPopulation .}
  optional{?s coy:hasGeometry ?geometry .}
  optional{?s coy:hasAlertLevel ?alertLevel .}
  optional{?s coy:hasEventStatus ?eventStatus .}
  optional{?s coy:hasIntensity ?intensity .}
  optional{?s coy:hasConfidence ?confidence .}
  optional{?s coy:hasResolution ?resolution .}
...
query_conflict = prefixes + """
SELECT DISTINCT ?countryLocation ?comment ?fatalities ?location ?actor ?timestamp ?label WHERE {
  ?s a coy:Conflict .
  ?s coy:hasCountryLocation ?countryLocation .
  ?s rdfs:comment ?comment .
  ?s coy:hasFatalities ?fatalities .
  ?s coy:hasLocation ?location .
  ?s coy:hasActor ?actor .
  ?s coy:hasTimestamp ?timestamp .
  ?s rdfs:label ?label .
}
"""

construct_query_conflict = prefixes + """
CONSTRUCT { ?s a coy:Conflict .
  ?s coy:hasCountryLocation ?countryLocation .
  # ?s rdfs:comment ?comment .
  ?s coy:hasFatalities ?fatalities .
  # ?s coy:hasLocation ?location .
...
construct_query_disaster = prefixes + """
CONSTRUCT {?s a coy:Disaster .
  ?s coy:hasCountryLocation ?countryLocation .
  ?s coy:hasStartTimestamp ?hasStartTimestamp .
  ?s coy:hasEndTimestamp ?hasEndTimestamp .
  ?s coy:hasEventType ?eventType .
  ?s coy:hasAlertScore ?alertScore .
  ?s coy:hasAffectedPopulation ?affectedPopulation .
  ?s coy:hasAlertLevel ?alertLevel .
...
#  ?s rdfs:label ?label .
  ...
  ?s coy:hasCountryLocation ?countryLocation .
  ?s coy:hasStartTimestamp ?hasStartTimestamp .
  optional{?s coy:hasEndTimestamp ?hasEndTimestamp .}
  optional{?s coy:hasEventType ?eventType .}
  optional{?s coy:hasAlertScore ?alertScore .}
  optional{?s coy:hasAffectedPopulation ?affectedPopulation .}
  optional{?s coy:hasAlertLevel ?alertLevel .}
...
construct_query_company = prefixes + """
CONSTRUCT{ ?s a coy:Company .
  # ?s coy:hasCountryLocation ?hasCountryLocation .
  # ?s coy:hasFoundingDate ?hasFoundingDate .
  # ?s coy:hasIndustry ?hasIndustry .
  ?s coy:isActive ?isActive .
  ?s coy:hasCountry ?hasCountry .
  # ?countryCode coy:hasIso2Code ?hasCountry .

  # ?s coy:hasSirenId ?hasSirenId .
  # ?s coy:hasSiretId ?hasSiretId .
  # ?s rdfs:label ?label .
  # ?s coy:hasCity ?hasCity .
  # ?s coy:hasLeiCode ?hasLeiCode .
  # ?s coy:hasName ?hasName .
  # ?s coy:hasAlternateName ?hasAlternateName .
  # ?s skos:altLabel ?altLabel
  } WHERE {
  ?s a coy:Company .
  # ?s coy:hasCountryLocation ?hasCountryLocation .
  # optional{?s coy:hasFoundingDate ?hasFoundingDate .}
  # optional{?s coy:hasIndustry ?hasIndustry .}
  ?s coy:isActive ?isActive .
  ?s coy:hasCountry ?hasCountry .
  ?countryCode coy:hasIso2Code ?hasCountry .

  # optional{?s coy:hasSirenId ?hasSirenId .}
  # optional{?s coy:hasSiretId ?hasSiretId .}
  # optional{?s rdfs:label ?label .}
  # optional{?s coy:hasCity ?hasCity .}
  # optional{?s coy:hasLeiCode ?hasLeiCode .}
  # optional{?s coy:hasName ?hasName .}
  # optional{?s coy:hasAlternateName ?hasAlternateName .}
  # optional{?s skos:altLabel ?altLabel .}
}
"""

query_company = prefixes + """
SELECT DISTINCT ?iso2Code ?countryCode WHERE {
  ?s a coy:Company .
  ?s coy:isActive ?isActive .
  ?s coy:hasCountry ?iso2Code .
  ?countryCode coy:hasIso2Code ?iso2Code .
}
"""


construct_query_SPEvent = prefixes + """
CONSTRUCT {?s a coy:SocioPoliticalEvent .
  ?s coy:hasCountryLocation ?hasCountryLocation .
  #?s coy:hasStoryId ?hasStoryId .
...
WHERE {
  ...
  optional{?s coy:hasCountryLocation ?hasCountryLocation .}
